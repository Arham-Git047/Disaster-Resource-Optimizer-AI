...
const NeedsList = () => {
  ...
  const unverifiedNeeds = Array.isArray(needs) ? needs.filter((need) => need.status === 'unverified') : [];

  if (unverifiedNeeds.length === 0) {
    return (
      <Alert severity="success" sx={{ mt: 2 }}>
        <Typography variant="h6">✅ All reports verified!</Typography>
  ...
  return (
    <Box>
      ...
      {unverifiedNeeds.map((need) => (
        <NeedCard key={need._id || need.id} need={need} />
      ))}
    </Box>
  );
};
